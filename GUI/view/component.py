...
def menu_label(master, title: str):
    return tk.Label(master, text=title, padx=20, pady=20, font='Helvetica 16 bold').pack()
...
def option_button(master, title, click_function):
    return tk.Button(master, text=title, bg='black', fg='white',
                     font='Helvetica 14', command=click_function, height=2).pack()


def exit_button(master, title, click_function):
    return tk.Button(master, text=title, bg='red', fg='white',
                     font='Helvetica 14', command=click_function, height=2).pack()
